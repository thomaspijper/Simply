...
class readInput(object):
    ...
    def readGeneral(self, file):
        ...
        'syncmethod':        'simtime',  # String, not required, default is 'simtime'
        'syncsimtime':       5,          # Int, not required, default is 5 (seconds)
        'syncevents':        500000,     # Int, not required, default is  5E5 (events)
        ...
        reqLength = [2]
        reqParams = ['particlecount','monomernames','maxwalltime']

        # Read lines of interest, store the parameters they contain in list
        inputParams = []
        rangeTuple = self.locateGroup(file,'general')
        if rangeTuple == None:
            self.missingGroupError('general')
        for i in range(rangeTuple[0]+1,rangeTuple[1]):
     ...
     # Check if the specified input is allowed
        ...
        # Check if syncmethod has a valid value
        if generalDict['syncmethod'] not in ['simtime','events']:
            print('\nInvalid sync method "{0}" specified'.format(generalDict['syncmethod']))
            sys.exit()
        ...
        if generalDict['starttemperature'] < 0.0:
            ...
            sys.exit()
        elif generalDict['starttemperature'] != 0.0 and generalDict['coolingrate'] == 0.0:
            print('\nError: the start temperature differs from base temperature, but no cooling rate was specified')
            sys.exit()
    ...
    def readMolecules(self, file, generalDict):
        ...
        rangeTuple = self.locateGroup(file,'molecules')
        if rangeTuple == None:
    ...
    def readReactions(self, file, moleculesList, ratesList):
        ...
        rangeTuple = self.locateGroup(file,'reactions')
        if rangeTuple == None:
        ...
        for i in range(rangeTuple[0]+1,rangeTuple[1]):
            ...
            if len(line) == 6:
                ...
                self.checkSpecies(linecache.getline(file, i), line[3], 'r', moleculesList) # Check reactant1
                self.checkSpecies(linecache.getline(file, i), line[5], 'p', moleculesList) # Check product1
            ...
            # Parse combinations and decompositions
                ...
                self.checkSpecies(linecache.getline(file, i), line[5], role, moleculesList) # Check reactant2/product1
                self.checkSpecies(linecache.getline(file, i), line[7], 'p', moleculesList) # Check product1/product2
            ...
            # Parse transfers
                ...
                if (line[4] != '+') or (line[6] != '->') or (line[8] != '+'):
                    self.lineError(linecache.getline(file, i),'Invalid reaction syntax')
                self.checkSpecies(linecache.getline(file, i), line[3], 'r', moleculesList) # Check reactant1
                self.checkSpecies(linecache.getline(file, i), line[5], 'r', moleculesList) # Check reactant2
                self.checkSpecies(linecache.getline(file, i), line[7], 'p', moleculesList) # Check product1
                self.checkSpecies(linecache.getline(file, i), line[9], 'p', moleculesList) # Check product2
    ...
    def readRateCoeffs(self, file, generalDict, moleculesList):
        ...
        # Initialise defaults
        reqLength = [3,4,9,10,6,7]
        reqTypes = ['f','a','fd','ad','fps','aps'] # types correspond with above required lengths ('f' has required length 3, etc.)
        rateCoeffs = []

        # Read lines of interest, store each rate in the correct list
        rangeTuple = self.locateGroup(file,'rates')
        if rangeTuple == None:
        ...
        for i in range(rangeTuple[0]+1,rangeTuple[1]):
            ...
            if (line[1] in ['a','ad','aps']) and (generalDict['temperature'] == 0.0):
                if moleculesList[j][1] == line[0]:
            ...
            if (line[1] in ['fps','aps']) and (generalDict['freevolume'] == 0):
                generalDict['freevolume'] = 1

            # Should we enable the recalculation of the conversion at every iteration?
            if (line[1] in ['fd','af','fps','aps']) and (generalDict['recalcconversion'] == 0):
    ...
    def readEnthalpies(self, file, generalDict, reactionsList):
        ...
        # Read lines of interest, store each rate in the correct list
        rangeTuple = self.locateGroup(file,'enthalpies')
        if rangeTuple == None:
    ...
    def readFreeVolume(self, file, generalDict, ratesList):
        ...
        # Read lines of interest, store each rate in the correct list
        rangeTuple = self.locateGroup(file,'freevolume')
        if rangeTuple == None:
    ...
    def locateGroup(self, file, group):
